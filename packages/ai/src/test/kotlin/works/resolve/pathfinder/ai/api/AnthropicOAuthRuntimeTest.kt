...
class AnthropicOAuthRuntimeTest {
    ...
    private fun provider(baseUrl: String): CatalogProvider = CatalogProvider(
        id = "anthropic",
        name = "Anthropic",
    ...
    private val sseBody = sequence {
        yield(
            "message_start" to
                """{"type":"message_start","message":{"id":"msg_oauth","model":"claude-sonnet-4-5","usage":{"input_tokens":9,"output_tokens":0}}}"""
        )
        yield(
    ...
    @Test
    fun `expired stored anthropic oauth credential refreshes with rotation and streams as bearer`() =
        runTest {
            ...
            val refreshBody = Json.parseToJsonElement(
