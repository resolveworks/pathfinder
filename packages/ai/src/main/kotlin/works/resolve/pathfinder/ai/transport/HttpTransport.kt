...
data class TransportRequest(
    ...
    override fun toString(): String = "TransportRequest(url=$url, bearerToken=" +
        (bearerToken?.let { "<redacted>" } ?: "null") +
        ", headers=${headers.keys}, body=<${body.size} bytes>" +
        ", timeoutMs=$timeoutMs)"

    override fun equals(other: Any?): Boolean = other is TransportRequest &&
        other.url == url &&
    ...
    override fun hashCode(): Int =
        31 * (31 * (31 * url.hashCode() + bearerToken.hashCode()) + headers.hashCode()) +
...
data class SseEvent(
    val data: String,
    /** SSE `event:` field name, when the server sent one. */
