...
import com.aletheia.ai.core.Usage
...
private fun estimateTextAndImageChars(content: List<Content>): Int =
    content.sumOf { block ->
        when (block.type) {
            ContentType.TEXT -> (block as com.aletheia.ai.core.TextContent).text.length
    ...
    return when (message.role) {
        ...
        MessageRole.ASSISTANT -> {
            ...
            for (block in message.contentList()) {
                when (block) {
                    is com.aletheia.ai.core.TextContent -> chars += block.text.length
                    is com.aletheia.ai.core.ThinkingContent -> chars += block.thinking.length
                    is com.aletheia.ai.core.ToolCall ->
...
private fun Message.contentList(): List<Content> = when (this) {
    is com.aletheia.ai.core.UserMessage -> content
    is com.aletheia.ai.core.AssistantMessage -> content
    is com.aletheia.ai.core.ToolResultMessage -> content
}
...
private fun getLastAssistantUsageInfo(messages: List<Message>): Pair<Usage, Int>? {
    ...
    for (i in messages.indices) {
        ...
        if (message.role == MessageRole.ASSISTANT) {
            val assistant = message as com.aletheia.ai.core.AssistantMessage
