...
import works.resolve.distill.ai.api.GoogleRequest.CommonOptions
import works.resolve.distill.ai.api.GoogleRequest.GoogleThinking
 ...
 *
 * Upstream drives the `@google/genai` SDK; Distill implements the same wire
...
class GoogleGenerativeAiApi(
    ...
    ): Flow<works.resolve.distill.ai.core.AssistantMessageEvent> {
    ...
    private fun missingApiKeyFlow(model: Model) = kotlinx.coroutines.flow.flow {
        val message = works.resolve.distill.ai.core.AssistantMessage(
            content = emptyList(),
            api = model.api,
            provider = model.provider,
            model = model.id,
            stopReason = works.resolve.distill.ai.core.StopReason.ERROR,
            errorMessage = "No API key for provider: ${model.provider}",
            timestamp = nowMs(),
        )
        emit(
            works.resolve.distill.ai.core.AssistantMessageEvent.Error(
                works.resolve.distill.ai.core.StopReason.ERROR,
                message,
            ),
        )
    }
