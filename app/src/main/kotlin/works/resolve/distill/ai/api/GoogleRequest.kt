 ...
 *
 * Wire shape: where pi hands `config` to the `@google/genai` SDK, Distill
...
object GoogleRequest {
    ...
    /**
     * User-Agent default pi sends via getPiUserAgent(); Distill identifies
     * itself (divergence: the Android client is not pi).
     */
    const val USER_AGENT = "distill (Android)"
}
