...
private fun estimateTextAndImageChars(content: List<Content>): Int =
    content.sumOf { block ->
        when (block.type) {
            ContentType.TEXT -> (block as works.resolve.distill.ai.core.TextContent).text.length
    ...
    return when (message.role) {
        ...
        MessageRole.ASSISTANT -> {
            ...
            for (block in message.contentList()) {
                when (block) {
                    is works.resolve.distill.ai.core.TextContent -> chars += block.text.length
                    is works.resolve.distill.ai.core.ThinkingContent -> chars += block.thinking.length
                    is works.resolve.distill.ai.core.ToolCall ->
...
private fun Message.contentList(): List<Content> = when (this) {
    is works.resolve.distill.ai.core.UserMessage -> content
    is works.resolve.distill.ai.core.AssistantMessage -> content
    is works.resolve.distill.ai.core.ToolResultMessage -> content
}
...
private fun getLastAssistantUsageInfo(messages: List<Message>): Pair<Usage, Int>? {
    ...
    for (i in messages.indices) {
        ...
        if (message.role == MessageRole.ASSISTANT) {
            val assistant = message as works.resolve.distill.ai.core.AssistantMessage
