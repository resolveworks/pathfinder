...
import works.resolve.distill.ai.core.mergeHeaders
 ...
 * through Distill's [works.resolve.distill.ai.transport.HttpStreamingTransport]
 ...
 * Distill convention in OpenAiCompletionsApi).
 */
...
data class OpenAiResponsesOptions(
    ...
    val maxRetryDelayMs: Long = works.resolve.distill.ai.core.StreamOptions.DEFAULT_MAX_RETRY_DELAY_MS,
...
fun splitDeferredTools(context: Context, enabled: Boolean): DeferredToolPlacement {
    ...
    for (message in context.messages) {
        when (message) {
            is works.resolve.distill.ai.core.AssistantMessage ->
                message.content.filterIsInstance<works.resolve.distill.ai.core.ToolCall>()
                    .forEach { usedNames.add(it.name) }
            is works.resolve.distill.ai.core.ToolResultMessage ->
...
/** pi's getPiUserAgent shape, adapted for Distill's Android runtime. */
...
class OpenAiResponsesApi(
    private val transport: works.resolve.distill.ai.transport.HttpStreamingTransport,
    ...
    override fun streamSimple(
        ...
        val clamped = options.reasoning?.let {
            works.resolve.distill.ai.core.clampThinkingLevel(model, toModelThinkingLevel(it))
        }
        ...
        return stream(
            ...
            OpenAiResponsesOptions(
                ...
                maxTokens = works.resolve.distill.ai.utils.clampMaxTokensToContext(
                    model,
                    context,
