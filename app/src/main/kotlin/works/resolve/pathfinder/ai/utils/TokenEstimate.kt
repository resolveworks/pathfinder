...
private fun estimateTextAndImageChars(content: List<Content>): Int =
    content.sumOf { block ->
        when (block.type) {
            ContentType.TEXT -> (block as works.resolve.pathfinder.ai.core.TextContent).text.length
...
private fun Message.contentList(): List<Content> = when (this) {
    is works.resolve.pathfinder.ai.core.UserMessage -> content
    is works.resolve.pathfinder.ai.core.AssistantMessage -> content
    is works.resolve.pathfinder.ai.core.ToolResultMessage -> content
}
...
private fun getLastAssistantUsageInfo(messages: List<Message>): Pair<Usage, Int>? {
    ...
    for (i in messages.indices) {
        ...
        if (message.role == MessageRole.ASSISTANT) {
            val assistant = message as works.resolve.pathfinder.ai.core.AssistantMessage
