...
private fun downgradeUnsupportedImages(messages: List<Message>, model: Model): List<Message> =
    if (model.input.contains(works.resolve.aletheia.ai.core.InputModality.IMAGE)) {
        messages
    } else {
        messages.map { msg ->
            when (msg.role) {
                MessageRole.USER -> {
                    val user = msg as works.resolve.aletheia.ai.core.UserMessage
    ...
    for (msg in transformed) {
        when (msg.role) {
            MessageRole.ASSISTANT -> {
                ...
                val assistantMsg = msg as AssistantMessage
                if (assistantMsg.stopReason == works.resolve.aletheia.ai.core.StopReason.ERROR ||
                    assistantMsg.stopReason == works.resolve.aletheia.ai.core.StopReason.ABORTED
                ) {
                    continue
                }
