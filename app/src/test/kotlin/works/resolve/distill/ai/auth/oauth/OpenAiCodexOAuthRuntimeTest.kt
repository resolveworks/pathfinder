...
import works.resolve.distill.ai.auth.InMemoryCredentialStore
import works.resolve.distill.ai.auth.NoopAuthContext
import works.resolve.distill.ai.auth.OAuthCredential
import works.resolve.distill.ai.auth.resolveProviderAuth
import works.resolve.distill.ai.auth.CatalogAuthProviderRef
...
import works.resolve.distill.ai.transport.OkHttpTransport
...
class OpenAiCodexOAuthRuntimeTest {
    ...
    fun `stored oauth credential authenticates an openai-codex-responses request`() {
        ...
        try {
            ...
            val events = runBlocking {
                models.stream(
                    model.copy(baseUrl = works.resolve.distill.ai.providers.normalizeBaseUrl(server.url("/backend-api").toString())),
     ...
     * [works.resolve.distill.ai.auth.ProviderAuthService.authMethods]): the
    ...
    fun `openai-codex projects a single subscription oauth method`() {
        ...
        val authService = works.resolve.distill.ai.auth.ProviderAuthService(
            catalog,
            works.resolve.distill.ai.auth.ProductionCatalogAuthRegistry,
            InMemoryCredentialStore(),
        )
        val methods = authService.authMethods("openai-codex")
        assertEquals(1, methods.size)
        assertEquals(works.resolve.distill.ai.auth.AuthType.OAUTH, methods.single().type)
