...
class CatalogProviderAuthTest {
    ...
    fun `explicit key and env shape cloudflare headers without reading the store`() = runTest {
        ...
        val auth = works.resolve.distill.agent.catalogAuthResolver(TestCatalogs.CLOUDFLARE, store, ctx)(
            "cf-explicit-key",
    ...
    fun `incomplete explicit cloudflare key resolves unconfigured`() = runTest {
        val auth = works.resolve.distill.agent.catalogAuthResolver(
    ...
    fun `api key auth name falls back to provider name plus API key without a label`() {
        ...
        val unlabeled = CatalogProvider(
            ...
            auth = works.resolve.distill.ai.providers.ProviderAuth(
                prompts = listOf(works.resolve.distill.ai.providers.AuthPrompt("UNLABELED_API_KEY", "Key")),
            ),
            models = emptyList(),
        )
