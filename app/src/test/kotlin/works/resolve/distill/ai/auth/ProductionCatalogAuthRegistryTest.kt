...
import works.resolve.distill.ai.auth.oauth.GitHubCopilotOAuthAuth
import works.resolve.distill.ai.auth.oauth.KimiCodingOAuthAuth
import works.resolve.distill.ai.auth.oauth.OpenAiCodexOAuthAuth
import works.resolve.distill.ai.auth.oauth.OpenRouterOAuthAuth
import works.resolve.distill.ai.auth.oauth.XaiOAuthAuth
import works.resolve.distill.ai.providers.CatalogProvider
import works.resolve.distill.ai.providers.ProviderCatalog

/** Verifies Distill's concrete registry uses pi's exact static provider ids. */
class ProductionCatalogAuthRegistryTest {
    ...
    private fun model(id: String) = works.resolve.distill.ai.core.Model(
        id = id,
        name = id,
    ...
    private fun provider(id: String, models: List<works.resolve.distill.ai.core.Model> = emptyList()) = CatalogProvider(
        id = id,
        name = id,
        baseUrl = "https://example.invalid",
        models = models,
