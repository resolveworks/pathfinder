...
class ProviderCatalogTest {
    ...
    fun `real asset zai glm-4_7 matches the retired hand-written ZaiModels port`() {
        ...
        val expected = Model(
            ...
            cost = works.resolve.distill.ai.core.ModelCost(
                input = 0.6,
                output = 2.2,
                cacheRead = 0.11,
                cacheWrite = 0.0,
            ...
            maxTokens = 131_072,
            compat = works.resolve.distill.ai.core.OpenAiCompletionsCompat(
                supportsStore = false,
                supportsDeveloperRole = false,
                supportsReasoningEffort = false,
    ...
    fun `compat anthropic flags map with pi defaults when absent`() {
        ...
        assertEquals(works.resolve.distill.ai.core.AnthropicMessagesCompat(), model.anthropicCompat)
    }
    ...
    fun `compat anthropic flags map explicit non-default values`() {
        ...
        assertEquals(
            works.resolve.distill.ai.core.AnthropicMessagesCompat(
                supportsEagerToolInputStreaming = false,
                supportsLongCacheRetention = false,
                sendSessionAffinityHeaders = true,
                supportsCacheControlOnTools = false,
    ...
    fun `responses compat parses from the catalog asset`() {
        ...
        assertEquals(
            works.resolve.distill.ai.core.SessionAffinityFormat.OPENAI_NOSESSION,
