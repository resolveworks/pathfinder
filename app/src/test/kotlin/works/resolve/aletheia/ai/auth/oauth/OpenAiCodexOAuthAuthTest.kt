...
class OpenAiCodexOAuthAuthTest {
    ...
    private val fixedVerifier = works.resolve.aletheia.ai.auth.PkceGenerator.base64url(rawVerifierBytes)
    ...
    private fun fixedPkce() = works.resolve.aletheia.ai.auth.PkceGenerator { rawVerifierBytes }
    ...
    fun `browser login notifies the auth url and exchanges a pasted code`() = runTest {
        ...
        assertEquals(
            "https://auth.openai.com/oauth/authorize" +
                ...
                "&code_challenge=${works.resolve.aletheia.ai.auth.PkceGenerator.challengeFor(fixedVerifier)}" +
