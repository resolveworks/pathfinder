...
class SessionStateRecordsTest {
    ...
    private fun started(seq: Long, id: String, lane: String = "main") =
        SessionMutation.Record(LaneRecord.OperationStartedRecord(id = id, lane = lane, seq = seq, timestamp = seq, intent = OperationIntent.run()))

    private fun finished(seq: Long, runId: String, outcome: OperationOutcome = OperationOutcome.COMPLETED) =
        SessionMutation.Record(
            LaneRecord.OperationFinishedRecord(id = "f$seq", lane = "main", seq = seq, timestamp = seq, runId = runId, outcome = outcome),
        )

    private fun usageRecord(seq: Long, usage: Usage) =
        SessionMutation.Record(
            LaneRecord.UsageRecord(
                id = "u$seq", lane = "main", seq = seq, timestamp = seq, usage = usage,
                fields = kotlinx.serialization.json.JsonObject(mapOf("cause" to kotlinx.serialization.json.JsonPrimitive("assistant"))),
            ),
        )

    private fun usage(input: Int = 10, cacheRead: Int = 100, cacheWrite: Int = 20, totalTokens: Int, costTotal: Double) =
        Usage(input = input, output = 5, cacheRead = cacheRead, cacheWrite = cacheWrite, reasoning = 0, totalTokens = totalTokens, cost = Cost(0.0, 0.0, 0.0, 0.0, costTotal))
    ...
    fun `usage records accumulate session stats incrementally`() {
        ...
        state.applyMutation(
            SessionMutation.Entry(
                lane = "main",
                entry = MessageEntry(id = "m1", seq = 1, parentId = null, timestamp = 1, message = works.resolve.pathfinder.ai.core.UserMessage.ofText("hi", 1)),
            ),
        )
    ...
    fun `operation records fold intent and sourceLeafId verbatim`() {
        ...
        val record = LaneRecord.OperationStartedRecord(
            id = "op1", lane = "main", seq = 1, timestamp = 1, sourceLeafId = "later-entry", intent = OperationIntent.compaction("e9"),
        )
