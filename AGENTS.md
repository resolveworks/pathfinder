...
A minimal native Android client for [pi](https://pi.dev). Aletheia ports the
...
- Pi is the behavioral source of truth. Mirror the current implementation in
  ...
  handling rather than designing Aletheia-specific equivalents.
...
- `works.resolve.aletheia.ai` mirrors `packages/ai`.
- `works.resolve.aletheia.agent` mirrors `packages/agent`.
...
- `AletheiaApplication` is the manual composition root. Keep the dependency
